...
export class ProdutoController {
  ...
  async listar(req: Request) {
    ...
    try {
      ...
      return NextResponse.json({ data, message: "Listagem de produtos" });
    } catch {
  ...
  async criar(req: Request) {
    ...
    const parse = produtoCreateSchema.safeParse(body);
    if (!parse.success) {
    ...
    try {
      ...
      return NextResponse.json({ data: created, message: "Produto criado" }, { status: 201 });
    } catch (err) {
